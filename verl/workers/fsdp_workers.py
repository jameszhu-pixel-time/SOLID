    ...
    def init_model(self):
        ...
        if self._is_ref:
            ...
            if self._is_actor:
                ...
                teacher_kl_ref = (
                    self.config.actor.get("use_kl_loss", False)
                    and self.config.actor.get("kl_ref_log_prob_source", "reference") == "teacher"
                )
                if self_distillation_cfg is not None and (loss_mode == "sdpo" or teacher_kl_ref):
