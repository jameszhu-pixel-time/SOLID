...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _update_teacher(self) -> None:
        """ema logic: teacher remains saperate
           Trust Region
        """
        
        self_distillation_cfg = getattr(self.config, "self_distillation", None)
        loss_mode = self.config.policy_loss.get("loss_mode", "vanilla")
        if not self_distillation_cfg or (loss_mode != "sdpo" and not self._uses_teacher_kl_ref()):
            return
    ...
    def _uses_teacher_kl_ref(self) -> bool:
        return bool(
            self.config.get("use_kl_loss", False)
            and self.config.get("kl_ref_log_prob_source", "reference") == "teacher"
        )
    ...
    def update_policy(self, data: DataProto):
        ...
        pad_token_id = data.meta_info.get("pad_token_id", 0)
        loss_mode = self.config.policy_loss.get("loss_mode", "vanilla")
        bootstrap_kl = self.config.get("bootstrap_kl", None)
        self_distillation_enabled = loss_mode == "sdpo"
        teacher_kl_ref_enabled = self._uses_teacher_kl_ref()
        self_distillation_cfg = getattr(self.config, "self_distillation", None)
        teacher_required_keys = set()
        if self_distillation_enabled or teacher_kl_ref_enabled:
            ...
            assert teacher_required_keys.issubset(set(data.batch.keys())), f"Missing required keys: {teacher_required_keys - set(data.batch.keys())}"
        ...
        if self.config.use_kl_loss and not teacher_kl_ref_enabled:
            select_keys.append("ref_log_prob")
        if self_distillation_enabled or teacher_kl_ref_enabled:
            select_keys.extend(list(teacher_required_keys))
        if teacher_kl_ref_enabled and "teacher_kl_mask" in data.batch.keys():
            select_keys.append("teacher_kl_mask")
        # Bootstrap KL stage weights (pre-computed in trainer)
        if self.config.get("bootstrap_kl", False) and "kl_stage_weights" in data.batch.keys():
            select_keys.append("kl_stage_weights")
        if self.config.get("bootstrap_kl", False) and "kl_stage_ids" in data.batch.keys():
            select_keys.append("kl_stage_ids")
        ...
        teacher_kl_all_masked_total = 0.0
        teacher_kl_microbatch_total = 0
        did_update = False
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    outputs = self._forward_micro_batch( #forward function
                        model_inputs,
                        temperature=temperature,
                        calculate_entropy=calculate_entropy,
                    ...
                    if self_distillation_enabled:
                        ...
                        with torch.no_grad():
                            # Stage weights are applied in KL loss computation below
                        ...
                        teacher_log_prob_for_kl = teacher_log_prob.detach()
                        ...
                        pg_loss, pg_metrics = compute_self_distillation_loss(
                            ...
                            kl_stage_weights=model_inputs.get("kl_stage_weights", None)
                            if self.config.get("bootstrap_kl", False)
                            else None,
                            kl_stage_ids=model_inputs.get("kl_stage_ids", None)
                            if self.config.get("bootstrap_kl", False)
                            else None,
                            kl_stage_flip_tau=self.config.get("bootstrap_kl_flip_tau", 1.0),
                            debug_kl_vector=self.config.get("bootstrap_kl_debug", False),
                            debug_kl_vector_tokens=self.config.get("bootstrap_kl_debug_tokens", 128),
                        )
                    ...
                    if self.config.use_kl_loss:
                        ...
                        teacher_kl_all_masked = 0.0
                        if teacher_kl_ref_enabled:
                            if teacher_log_prob_for_kl is None:
                                teacher_inputs = {
                                    "responses": model_inputs["responses"],
                                    "input_ids": model_inputs["teacher_input_ids"],
                                    "attention_mask": model_inputs["teacher_attention_mask"],
                                ...
                                if teacher_model is None:
                                    raise ValueError("teacher KL reference requires a separate teacher_module.")
                                if teacher_regularization == "trust-region" and teacher_model is self.actor_module:
                                ...
                                ref_log_prob = teacher_outputs["log_probs"].detach()
                            else:
                                ref_log_prob = teacher_log_prob_for_kl
                            micro_batch_metrics["actor/kl_ref_source_teacher"] = 1.0
                            teacher_kl_mask = model_inputs.get("teacher_kl_mask", None)
                            if teacher_kl_mask is not None:
                                kl_loss_mask = response_mask * teacher_kl_mask.unsqueeze(1)
                                micro_batch_metrics["actor/teacher_kl_active_sample_fraction"] = (
                                    teacher_kl_mask.float().mean().detach().item()
                                )
                        else:
                        ...
                        if teacher_kl_ref_enabled and "self_distillation_mask" in model_inputs:
                            fallback_mask = model_inputs["self_distillation_mask"].unsqueeze(1)
                            kld = kld * fallback_mask
                            if "teacher_kl_mask" not in model_inputs:
                                kl_loss_mask = response_mask * fallback_mask
                        # Apply bootstrap KL stage weights 
                        if self.config.get("bootstrap_kl", False) and "kl_stage_weights" in model_inputs:
                            kld = kld * model_inputs["kl_stage_weights"]
                        if teacher_kl_ref_enabled:
                            kld = kld * kl_loss_mask
                        if self.config.get("bootstrap_kl", False) and "kl_stage_ids" in model_inputs:
                            kld, ref_flip_weights, ref_flip_metrics = apply_stage_kl_random_flip(
                                per_token_kl=kld,
                                loss_mask=kl_loss_mask,
                                stage_ids=model_inputs["kl_stage_ids"],
                                tau=self.config.get("bootstrap_kl_flip_tau", 1.0),
                            )
                            micro_batch_metrics.update({
                                f"actor/ref_{key}": value for key, value in ref_flip_metrics.items()
                            })
                        if self.config.get("bootstrap_kl_debug", False) and kld.shape[0] > 0:
                            valid_len = int(response_mask[0].sum().detach().item())
                            debug_tokens = self.config.get("bootstrap_kl_debug_tokens", 128)
                            show_len = valid_len if debug_tokens is None or debug_tokens <= 0 else min(valid_len, int(debug_tokens))
                            final_ref_kl_vec = kld[0, :show_len].detach().float().cpu().tolist()
                            print(
                                "[DEBUG] actor/ref_final_kl_vector "
                                f"valid_len={valid_len} shown={show_len} values={final_ref_kl_vec}"
                            )
                            if self.config.get("bootstrap_kl", False) and "kl_stage_ids" in model_inputs:
                                final_ref_weight_vec = ref_flip_weights[0, :show_len].detach().float().cpu().tolist()
                                print(
                                    "[DEBUG] actor/ref_final_kl_flip_weights "
                                    f"valid_len={valid_len} shown={show_len} values={final_ref_weight_vec}"
                                )
                        if teacher_kl_ref_enabled and kl_loss_mask.sum().detach().item() == 0:
                            kl_loss = kld.sum() * 0.0
                            teacher_kl_all_masked = 1.0
                        else:
                            kl_loss = agg_loss(loss_mat=kld, loss_mask=kl_loss_mask, loss_agg_mode=loss_agg_mode)
                        if teacher_kl_ref_enabled:
                            teacher_kl_all_masked_total += teacher_kl_all_masked
                            teacher_kl_microbatch_total += 1
        ...
        if teacher_kl_microbatch_total > 0:
            metrics["actor/teacher_kl_all_masked"] = teacher_kl_all_masked_total / teacher_kl_microbatch_total
        return metrics
