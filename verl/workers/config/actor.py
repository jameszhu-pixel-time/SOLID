...
from verl.or_utils.copt_prompt import copt_prompt_method2_lp
from verl.or_utils.gurobi_prompt import gurobi_prompt_method2_lp
...
class SelfDistillationConfig(BaseConfig):
    ...
    or_template_style: str = "gurobi"
    use_or_template: bool = False
    or_reprompt_template_gurobi: str = gurobi_prompt_method2_lp["user"]
    or_reprompt_template_copt: str = copt_prompt_method2_lp["user"]
    or_system_template_gurobi: str = gurobi_prompt_method2_lp["system"]
    or_system_template_copt: str = copt_prompt_method2_lp["system"]
    or_lp_template: str = (
        "```lp\n{lp_content}\n```\n"
    )
...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        kl_ref_log_prob_source (str): Source for KL reference log probabilities. Options: "reference", "teacher".
    ...
    kl_ref_log_prob_source: str = "reference"
    bootstrap_kl: bool = False
    bootstrap_kl_style: str = "gurobi"
    bootstrap_kl_decay: str = "linear"  # linear | exponential | uniform | problem_info | random_flipped | random_stage | solver_info
    bootstrap_kl_gamma: float = 2.0  # for exponential decay: weight = gamma^(stage_idx / num_stages)
    bootstrap_kl_flip_tau: float = 1.0
    bootstrap_kl_random_mask_prob: float = 0.5
    bootstrap_kl_random_seed: int = 42
    bootstrap_kl_debug: bool = False
    bootstrap_kl_debug_tokens: int = 128
    ppo_epochs: int = 1
    shuffle: bool = False
    ...
    def __post_init__(self):
        ...
        if self.bootstrap_kl_decay not in valid_bootstrap_kl_decays:
            raise ValueError(
                f"Invalid bootstrap_kl_decay: {self.bootstrap_kl_decay}. "
                f"Expected one of {sorted(valid_bootstrap_kl_decays)}."
            )
        if not 0.0 <= self.bootstrap_kl_random_mask_prob <= 1.0:
            raise ValueError(
                "bootstrap_kl_random_mask_prob must be in [0, 1], "
                f"got {self.bootstrap_kl_random_mask_prob}."
            )
