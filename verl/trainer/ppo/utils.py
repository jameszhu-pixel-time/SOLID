...
def need_reference_policy(
    ...
    actor_use_kl_loss = actor_config.get("use_kl_loss", False)
    teacher_kl_ref = (
        actor_use_kl_loss
        and actor_config.get("kl_ref_log_prob_source", "reference") == "teacher"
    )
    return config.algorithm.use_kl_in_reward or (actor_use_kl_loss and not teacher_kl_ref)
