...
def apply_stage_kl_random_flip(
    per_token_kl: torch.Tensor,
    loss_mask: torch.Tensor,
    stage_ids: torch.Tensor,
    tau: float = 1.0,
) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
    """Flip stage KL signs using stage-level KL as the sampling distribution.

    Stage 0 is excluded from flipping because it usually corresponds to the
    problem description / prompt restatement and often has a large KL for
    reasons unrelated to useful exploration.
    """
    if stage_ids.shape != per_token_kl.shape:
        raise ValueError(f"stage_ids must match KL shape, got {stage_ids.shape=} vs {per_token_kl.shape=}")
    if loss_mask.shape != per_token_kl.shape:
        raise ValueError(f"loss_mask must match KL shape, got {loss_mask.shape=} vs {per_token_kl.shape=}")

    tau = max(float(tau), 1e-6)
    stage_ids = stage_ids.to(device=per_token_kl.device)
    loss_mask_bool = loss_mask.bool()
    flip_weights = torch.ones_like(per_token_kl)
    metrics = {}

    num_candidate_stages = 0
    num_flipped_stages = 0
    prob_means = []
    prob_maxes = []
    stage_kl_means = []

    with torch.no_grad():
        kl_for_score = per_token_kl.detach().abs()
        batch_size = per_token_kl.shape[0]
        for b in range(batch_size):
            valid_stage_ids = torch.unique(stage_ids[b][loss_mask_bool[b] & (stage_ids[b] >= 0)])
            candidate_stage_ids = valid_stage_ids[valid_stage_ids > 0].sort().values
            if candidate_stage_ids.numel() == 0:
                continue

            stage_kl_values = []
            kept_stage_ids = []
            for stage_id in candidate_stage_ids:
                stage_mask = loss_mask_bool[b] & (stage_ids[b] == stage_id)
                if stage_mask.any():
                    stage_kl_values.append(kl_for_score[b][stage_mask].mean())
                    kept_stage_ids.append(stage_id)
            if not stage_kl_values:
                continue

            stage_kl_tensor = torch.stack(stage_kl_values)
            flip_probs = torch.softmax(stage_kl_tensor / tau, dim=0)
            chosen_idx = torch.multinomial(flip_probs, num_samples=1).item()
            chosen_stage_id = kept_stage_ids[chosen_idx]
            flip_weights[b][stage_ids[b] == chosen_stage_id] = -1.0

            num_candidate_stages += len(kept_stage_ids)
            num_flipped_stages += 1
            prob_means.append(flip_probs.mean())
            prob_maxes.append(flip_probs.max())
            stage_kl_means.append(stage_kl_tensor.mean())

    metrics["bootstrap_kl_random_flip/candidate_stages"] = float(num_candidate_stages)
    metrics["bootstrap_kl_random_flip/flipped_stages"] = float(num_flipped_stages)
    metrics["bootstrap_kl_random_flip/flipped_fraction"] = (
        num_flipped_stages / num_candidate_stages if num_candidate_stages > 0 else float("nan")
    )
    metrics["bootstrap_kl_random_flip/prob_mean"] = (
        torch.stack(prob_means).mean().item() if prob_means else float("nan")
    )
    metrics["bootstrap_kl_random_flip/prob_max_mean"] = (
        torch.stack(prob_maxes).mean().item() if prob_maxes else float("nan")
    )
    metrics["bootstrap_kl_random_flip/stage_kl_mean"] = (
        torch.stack(stage_kl_means).mean().item() if stage_kl_means else float("nan")
    )

    return per_token_kl * flip_weights, flip_weights, metrics
...
def compute_self_distillation_loss(
    ...
    kl_stage_weights: Optional[torch.Tensor] = None,
    kl_stage_ids: Optional[torch.Tensor] = None,
    kl_stage_flip_tau: float = 1.0,
    debug_kl_vector: bool = False,
    debug_kl_vector_tokens: int = 128,
) -> tuple[torch.Tensor, dict[str, Any]]:
    ...
    if kl_stage_weights is not None:
        if kl_stage_weights.shape != per_token_loss.shape:
            raise ValueError(
                "kl_stage_weights must match per-token distillation KL shape, "
                f"got {kl_stage_weights.shape=} vs {per_token_loss.shape=}"
            )
        per_token_loss = per_token_loss * kl_stage_weights
        active_weights = kl_stage_weights[loss_mask.bool()]
        if active_weights.numel() > 0:
            metrics["self_distillation/bootstrap_kl_weight_mean"] = active_weights.mean().detach().item()
            metrics["self_distillation/bootstrap_kl_weight_min"] = active_weights.min().detach().item()
            metrics["self_distillation/bootstrap_kl_weight_max"] = active_weights.max().detach().item()
        else:
            metrics["self_distillation/bootstrap_kl_weight_mean"] = float("nan")
            metrics["self_distillation/bootstrap_kl_weight_min"] = float("nan")
            metrics["self_distillation/bootstrap_kl_weight_max"] = float("nan")

    dynamic_flip_weights = None
    if kl_stage_ids is not None:
        per_token_loss, dynamic_flip_weights, flip_metrics = apply_stage_kl_random_flip(
            per_token_kl=per_token_loss,
            loss_mask=loss_mask,
            stage_ids=kl_stage_ids,
            tau=kl_stage_flip_tau,
        )
        metrics.update({f"self_distillation/{key}": value for key, value in flip_metrics.items()})

    if debug_kl_vector and per_token_loss.shape[0] > 0:
        with torch.no_grad():
            valid_len = int(response_mask[0].sum().detach().item())
            if debug_kl_vector_tokens is None or debug_kl_vector_tokens <= 0:
                show_len = valid_len
            else:
                show_len = min(valid_len, int(debug_kl_vector_tokens))
            final_kl_vec = per_token_loss[0, :show_len].detach().float().cpu().tolist()
            print(
                "[DEBUG] self_distillation/final_kl_vector "
                f"valid_len={valid_len} shown={show_len} values={final_kl_vec}"
            )
            if kl_stage_weights is not None:
                weight_vec = kl_stage_weights[0, :show_len].detach().float().cpu().tolist()
                print(
                    "[DEBUG] self_distillation/final_kl_stage_weights "
                    f"valid_len={valid_len} shown={show_len} values={weight_vec}"
                )
            if dynamic_flip_weights is not None:
                flip_weight_vec = dynamic_flip_weights[0, :show_len].detach().float().cpu().tolist()
                print(
                    "[DEBUG] self_distillation/final_kl_flip_weights "
                    f"valid_len={valid_len} shown={show_len} values={flip_weight_vec}"
                )
