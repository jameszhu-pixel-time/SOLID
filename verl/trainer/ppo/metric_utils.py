...
def _valid_numeric_values(values: list[Any]) -> tuple[list[float], list[int]]:
    numeric_values = []
    valid_indices = []
    ...
    return numeric_values, valid_indices


def compute_rule_score_pass_rate_metrics(
    reward_tensor: torch.Tensor,
    reward_extra_infos_dict: dict[str, list[Any]] | None,
    prefix: str = "train",
) -> dict[str, float]:
    metrics = {}
    if reward_tensor is not None:
        sequence_scores = reward_tensor.sum(-1).detach().float().cpu().numpy().tolist()
        score_values, _ = _valid_numeric_values(sequence_scores)
        if score_values:
            metrics[f"{prefix}/score"] = float(np.mean(score_values))

    if reward_extra_infos_dict:
        pass_rate_values, _ = _valid_numeric_values(reward_extra_infos_dict.get("pass_rate", []))
        if pass_rate_values:
            metrics[f"{prefix}/pass_rate"] = float(np.mean(pass_rate_values))
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                var_vals, valid_indices = _valid_numeric_values(var_vals)
                if not var_vals:
                    continue
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        ...
                        if has_pred:
                            # create vote_data
                            vote_data = [
                                {"val": var_vals[pos], "pred": pred_vals[idx]}
    ...
    for data_source, var2metric2uid_vals in data_src2var2metric2uid_vals.items():
        for var_name, metric2uid_vals in var2metric2uid_vals.items():
            for metric_name, uid_vals in metric2uid_vals.items():
                uid_vals, _ = _valid_numeric_values(uid_vals)
                if uid_vals:
