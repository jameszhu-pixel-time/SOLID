...
from verl.trainer.ppo.metric_utils import (
    ...
    compute_rule_score_pass_rate_metrics,
...
from verl.trainer.ppo.teacher_prompt_utils import build_or_teacher_message
...
from verl.trainer.ppo import ttrl_utils
...
class RayPPOTrainer:
    ...
    @staticmethod
    def _normalize_or_style(style: Any) -> str:
        style = str(style or "gurobi").lower()
        return "copt" if style == "copt" else "gurobi"

    def _resolve_or_style(self) -> str:
        ttrl_cfg = self.config.get("ttrl", {})
        data_cfg = self.config.get("data", {})
        return self._normalize_or_style(
            ttrl_cfg.get("solver_name")
            or data_cfg.get("chat_template_style")
            or "gurobi"
        )
    ...
    @staticmethod
    def _maybe_collect_extra_info(
        batch: "DataProto",
        batch_size: int,
    ) -> list[dict]:
        """Collect per-sample extra_info and reward_model fields from batch.

        Returns a list of dicts (one per sample) with keys:
            lp_formulation, candidate_lp_formulation, solved_objective,
            code_exec_res, majority_cluster_member, majority_gt, original_gt
        All values default to None if not present.
        """
        ...
        for i in range(batch_size):
            entry = {
                "lp_formulation": None,
                "candidate_lp_formulation": None,
                "solved_objective": None,
                "code_exec_res": None,
                "majority_cluster_member": None,
                "majority_gt": None,
                "original_gt": None,
            }
            if extra_infos is not None:
                ei = extra_infos[i] if hasattr(extra_infos, '__getitem__') else {}
                if isinstance(ei, dict):
                    entry["lp_formulation"] = ei.get("lp_formulation") or None
                    entry["candidate_lp_formulation"] = ei.get("candidate_lp_formulation") or None
                    entry["solved_objective"] = ei.get("solved_objective") or None
                    entry["code_exec_res"] = ei.get("code_exec_res") or None
                    entry["majority_cluster_member"] = ei.get(
                        "majority_cluster_member"
                    )
            if reward_models is not None:
                rm = reward_models[i] if hasattr(reward_models, '__getitem__') else {}
                if isinstance(rm, dict):
                    entry["majority_gt"] = rm.get("majority_gt") or None
                    entry["original_gt"] = rm.get("original_gt") or None
    ...
    def _maybe_build_self_distillation_batch(
        ...
        teacher_kl_ref = (
            ...
            and self.config.actor_rollout_ref.actor.get("kl_ref_log_prob_source", "reference") == "teacher"
        )##new opsd-grpo logic here. use teacher to activate
        if self_distillation_cfg is None or (loss_mode != "sdpo" and not teacher_kl_ref):
            return None
        ...
        or_extra_info_list = self._maybe_collect_extra_info(batch, batch_size)
        if self.config.actor_rollout_ref.actor.get("bootstrap_kl_decay") == "solver_info":
            # Localization below uses the copied strings in or_extra_info_list.
            # Drop the per-rollout LP before the actor-update RPC to avoid
            # serializing large LP text after its only consumer is finished.
            for extra_info in batch.non_tensor_batch.get("extra_info", []):
                if isinstance(extra_info, dict):
                    extra_info.pop("candidate_lp_formulation", None)
        ...
        apply_or_distill = self.config.trainer.get("apply_OR_distill", False)
        if apply_or_distill:
            solution_strs = [
                or_extra_info_list[i]["lp_formulation"] for i in range(batch_size)
            ]
        else:
            solution_strs = [
                self._get_solution(
        ...
        def _build_or_teacher_message(i: int) -> list[dict]:
            """Build teacher message for OR distillation using LP formulation as reference."""
            lp_content = solution_strs[i]
            lp_selection_target = self.config.get("ttrl", {}).get("lp_selection_target", "voted_answer")
            reference_obj = (
                or_extra_info_list[i].get("original_gt")
                if lp_selection_target == "golden_answer"
                else or_extra_info_list[i].get("majority_gt")
            ) or ""
            or_style = self_distillation_cfg.get("or_template_style") or self._resolve_or_style()
            return build_or_teacher_message(
                prompt_text=prompt_texts[i],
                lp_content=lp_content,
                majority_gt=reference_obj,
                or_style=or_style,
                fallback_messages=batch.non_tensor_batch["raw_prompt"][i],
            )

        if apply_or_distill:
            print(f"Or construction")
            messages = [_build_or_teacher_message(i) for i in range(batch_size)]
        else:
        ...
        teacher_prompt_prefix_debug = self.tokenizer.apply_chat_template(
            messages[0],
            tokenize=False,
        ...
        print(
            "[DEBUG] Teacher prompt prefix for batch item 0 "
            f"(chars={len(teacher_prompt_prefix_debug)})\n"
            "===== BEGIN TEACHER PROMPT PREFIX =====\n"
            f"{teacher_prompt_prefix_debug}\n"
            "===== END TEACHER PROMPT PREFIX =====",
            flush=True,
        )
        ...
        bootstrap_kl = self.config.actor_rollout_ref.actor.get("bootstrap_kl", False)
        kl_stage_weights = None
        kl_stage_ids = None
        solver_info_metrics = {}
        random_stage_metrics = {}
        if bootstrap_kl:
            from verl.trainer.ppo.ttrl_utils import (
                build_solver_info_weight_mask,
                build_stage_id_mask,
                build_stage_weight_mask,
                compute_random_stage_mask_metrics,
                compute_solver_info_mask_metrics,
                compute_stage_boundaries,
            )
            bk_style = self.config.actor_rollout_ref.actor.get("bootstrap_kl_style") or self._resolve_or_style()
            bk_decay = self.config.actor_rollout_ref.actor.get("bootstrap_kl_decay", "linear")
            bk_gamma = self.config.actor_rollout_ref.actor.get("bootstrap_kl_gamma", 2.0)
            bk_random_mask_prob = self.config.actor_rollout_ref.actor.get(
                "bootstrap_kl_random_mask_prob",
                0.5,
            )
            bk_random_seed = self.config.actor_rollout_ref.actor.get(
                "bootstrap_kl_random_seed",
                42,
            )
            stage_bounds = compute_stage_boundaries(responses, self.tokenizer, style=bk_style)
            print(f"[DEBUG]: stage bounds: {stage_bounds}")
            if bk_decay == "random_flipped":
                kl_stage_ids = build_stage_id_mask(stage_bounds, responses.shape[1]).to(device)
            elif bk_decay == "random_stage":
                random_generator = torch.Generator(device="cpu")
                random_generator.manual_seed(
                    int(bk_random_seed) + int(getattr(self, "global_steps", 0))
                )
                kl_stage_weights = build_stage_weight_mask(
                    stage_bounds,
                    responses.shape[1],
                    decay=bk_decay,
                    random_mask_prob=float(bk_random_mask_prob),
                    generator=random_generator,
                ).to(device)
                random_stage_metrics = compute_random_stage_mask_metrics(
                    stage_bounds,
                    kl_stage_weights,
                    response_mask,
                )
                random_stage_metrics["bootstrap_kl/random_stage_seed"] = float(
                    int(bk_random_seed)
                    + int(getattr(self, "global_steps", 0))
                )
            elif bk_decay == "solver_info":
                candidate_lps = [
                    item.get("candidate_lp_formulation")
                    for item in or_extra_info_list
                ]
                reference_lps = [
                    item.get("lp_formulation")
                    for item in or_extra_info_list
                ]
                code_exec_results = [
                    item.get("code_exec_res")
                    for item in or_extra_info_list
                ]
                solver_info_started = time.perf_counter()
                kl_stage_weights, solver_info_results = build_solver_info_weight_mask(
                    stage_bounds,
                    responses.shape[1],
                    candidate_lps,
                    reference_lps,
                ...
                kl_stage_weights = kl_stage_weights.to(device)
                result_count = max(len(solver_info_results), 1)
                localized_count = sum(result.status == "localized" for result in solver_info_results)
                valid_response_tokens = response_mask.sum().clamp_min(1)
                enabled_token_fraction = float(
                    ((kl_stage_weights > 0).to(response_mask.dtype) * response_mask).sum().item()
                    / valid_response_tokens.item()
                )
                solver_info_metrics = {
                    "solver_info/localized_fraction": localized_count / result_count,
                    "solver_info/fallback_fraction": (len(solver_info_results) - localized_count) / result_count,
                    "solver_info/enabled_token_fraction": enabled_token_fraction,
                    "solver_info/localization_time_s": time.perf_counter() - solver_info_started,
                }
                solver_info_metrics.update(
                    compute_solver_info_mask_metrics(
                        stage_bounds,
                        solver_info_results,
                        kl_stage_weights,
                ...
                for reported_step in (3, 4, 5):
                    solver_info_metrics[f"solver_info/reported_step_{reported_step}_fraction"] = (
                        sum(result.reported_step == reported_step for result in solver_info_results)
                        / result_count
                    )
                for error_step in (3, 4, 5, 9):
                    solver_info_metrics[f"solver_info/error_step_{error_step}_fraction"] = (
                        sum(error_step in result.error_steps for result in solver_info_results)
                        / result_count
                    )
                fallback_reasons = (
                    "missing_candidate_lp",
                    "missing_reference_lp",
                    "parse_error",
                    "complexity_limit",
                    "invalid_stage_boundaries",
                    "invalid_reported_step",
                )
                for reason in fallback_reasons:
                    solver_info_metrics[f"solver_info/reason_{reason}_fraction"] = (
                        sum(result.fallback_reason == reason for result in solver_info_results)
                        / result_count
                    )
                    solver_info_metrics[f"solver_info/fallback_{reason}_fraction"] = (
                        sum(
                            result.status == "fallback"
                            and result.fallback_reason == reason
                            for result in solver_info_results
                        )
                        / result_count
                    )
                if self.config.actor_rollout_ref.actor.get("bootstrap_kl_debug", False) and solver_info_results:
                    debug_index = next(
                        (
                            index
                            for index, result in enumerate(solver_info_results)
                            if result.error_steps or result.status == "fallback"
                        ),
                        0,
                    )
                    first_result = solver_info_results[debug_index]
                    enabled_tokens = int(
                        ((kl_stage_weights[debug_index] > 0) * response_mask[debug_index].bool()).sum().item()
                    )
                    print(
                        f"[DEBUG] solver_info/sample_{debug_index} "
                        f"status={first_result.status} "
                        f"reported_step={first_result.reported_step} "
                        f"error_steps={first_result.error_steps} "
                        f"difference_count={first_result.difference_count} "
                        f"category_counts={first_result.category_counts} "
                        f"fallback_reason={first_result.fallback_reason} "
                        f"code_exec_res={code_exec_results[debug_index]!r} "
                        f"enabled_response_tokens={enabled_tokens}",
                        flush=True,
                    )
            else:
                kl_stage_weights = build_stage_weight_mask(stage_bounds, responses.shape[1], decay=bk_decay, gamma=bk_gamma).to(device)
        ...
        correct_mask = seq_scores >= self_distillation_cfg.success_reward_threshold
        from verl.trainer.ppo.ttrl_utils import build_teacher_kl_sample_mask

        (
            teacher_kl_mask,
            majority_cluster_mask,
            majority_membership_available,
        ) = build_teacher_kl_sample_mask(
            ...
            require_majority_membership=bool(
                self.config.get("ttrl", {}).get("enable", False)
                and self.config.get("ttrl", {}).get(
                    "voting_strategy",
                    "majority",
                )
                == "majority"
        ...
        print(f"[DEBUG] distillation mask:{self_distillation_mask} ")
        print(f"[DEBUG] teacher KL mask (majority samples masked out):{teacher_kl_mask} ")
        ...
        metrics = {
            ...
            "teacher_kl/correct_sample_fraction": correct_mask.float().mean().item(),
            "teacher_kl/majority_cluster_fraction": majority_cluster_mask.float().mean().item(),
            "teacher_kl/active_sample_fraction": teacher_kl_mask.float().mean().item(),
        }
        metrics.update(solver_info_metrics)
        metrics.update(random_stage_metrics)
        tensors_dict = {
        ...
        if kl_stage_weights is not None:
            tensors_dict["kl_stage_weights"] = kl_stage_weights
        if kl_stage_ids is not None:
            tensors_dict["kl_stage_ids"] = kl_stage_ids
        return DataProto.from_dict(tensors=tensors_dict), metrics
    ...
    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns):
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            core_vars = {"score", "pass_rate"} & set(var2metric2val)
            core_vars.add("acc" if "acc" in var2metric2val else "reward")
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.get("ttrl", {}).get("enable", False):
                        from verl.trainer.ppo.ttrl_utils import apply_ttrl_gt,extract_extra_info #vote here.
                        print(f'[DEBUG] non_tensor_batch keys {list(batch[0].non_tensor_batch.keys())}')
                        voting_strategy = self.config.get("ttrl", {}).get("voting_strategy", "majority")
                        solver_name = self._resolve_or_style()
                        use_or_template = self.config.get("ttrl", {}).get("use_or_template", False)
                        batch = apply_ttrl_gt(
                            batch,
                            gen_batch_output,
                            ...
                            extract_lp=use_or_template,
                        )
                        rollout_obj, rollout_sol, rollout_code, rollout_lp = extract_extra_info(batch) #fixed logic here
                        if use_or_template:
                            from verl.trainer.ppo.ttrl_utils import select_reference_lp_from_feedback
                            lp_selection_target = self.config.get("ttrl", {}).get("lp_selection_target", "voted_answer")
                            lp_contents = select_reference_lp_from_feedback(
                                batch,
                                self.config.actor_rollout_ref.rollout.n,
                                selection_target=lp_selection_target,
                            )
                            print(f"[DEBUG] lp_contents length {len(lp_contents)}")
                            for i in range(len(batch)):
                                batch[i].non_tensor_batch["extra_info"]["lp_formulation"] = lp_contents[i]
                        # The flat rollout_lp list is already retained on the
                        # driver; avoid copying every prompt-level LP list n
                        # times through DataProto.repeat().
                        for i in range(len(batch)):
                            batch[i].non_tensor_batch["extra_info"].pop(
                    ...
                    if self.config.get("ttrl", {}).get("enable", False):
                        ...
                        for i in range(len(batch)):
                            ...
                            prompt_cluster_members = extra_info.get(
                                "majority_cluster_members"
                            )
                            rollout_index = (
                                i % self.config.actor_rollout_ref.rollout.n
                            )
                            if (
                                isinstance(prompt_cluster_members, (list, tuple))
                                and rollout_index < len(prompt_cluster_members)
                            ):
                                extra_info["majority_cluster_member"] = bool(
                                    prompt_cluster_members[rollout_index]
                                )
                                reward_model_info = batch.non_tensor_batch[
                                    "reward_model"
                                ][i]
                                if isinstance(reward_model_info, dict):
                                    extra_info["majority_vote_target"] = (
                                        reward_model_info.get("majority_gt")
                                    )
                            extra_info["solved_objective"] = rollout_obj[i]
                            extra_info["solution"] = rollout_sol[i]
                            extra_info["code_exec_res"] = rollout_code[i]
                            extra_info["candidate_lp_formulation"] = rollout_lp[i]
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        metrics.update(compute_rule_score_pass_rate_metrics(reward_tensor, reward_extra_infos_dict))
                    ...
                    if self.config.get("ttrl", {}).get("enable", False):
                        ...
                        from verl.trainer.ppo.ttrl_utils import apply_original_gt, compute_ttrl_metrics, compute_multi_strategy_metrics
                        ...
                        # Compute per-strategy win rate comparison
                        strategy_metrics = compute_multi_strategy_metrics(batch, self.config.ttrl.n_samples_per_prompt)
