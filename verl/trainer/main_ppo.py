...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        teacher_kl_ref = (
            ...
            and config.actor_rollout_ref.actor.get("kl_ref_log_prob_source", "reference") == "teacher"
        )
        self_distillation_needs_ref = self_distillation_cfg is not None and (loss_mode == "sdpo" or teacher_kl_ref)
        ...
        if self_distillation_needs_ref and use_legacy_worker_impl == "disable":
            raise ValueError(
                "SDPO/teacher KL requires the legacy worker implementation to colocate the teacher."
            )
