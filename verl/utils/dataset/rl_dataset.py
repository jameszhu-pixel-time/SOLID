...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        dataframes = []
        chat_template_style = self.config.get("chat_template_style", None)
        ...
        if chat_template_style:
            from verl.or_utils import copt_prompt, gurobi_prompt
            if chat_template_style == "copt":
                template = copt_prompt.copt_prompt_temp
            else:
                template = gurobi_prompt.gurobi_prompt_temp
            prompt_key = self.prompt_key

            def _render_system_template(system_prompt):
                return system_prompt.strip().replace("{{", "{").replace("}}", "}")

            def _apply_or_template(doc):
                content = doc[prompt_key]
                if isinstance(content, list):
                    content = content[0]["content"]
                return {prompt_key: [
                    {"role": "system", "content": _render_system_template(template["system"])},
                    {"role": "user", "content": template["user"].replace("{question}", content).strip()},
                ]}

            self.dataframe = self.dataframe.map(_apply_or_template)
            print(f"Applied OR chat template style: {chat_template_style}, dataset len: {len(self.dataframe)}")
            print(f'[DEBUG] datset confirmed: {self.dataframe[0][prompt_key]}')
